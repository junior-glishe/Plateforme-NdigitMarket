...
(() => {
  ...
  /* ---------- 10. Touche Entrée pour lancer la recherche (toutes pages) ---------- */
  document.addEventListener('keydown', (e) => {
    if (e.key === 'Enter') {
      const target = e.target;
      const filterForm = target.closest('form[data-filter]');
      if (filterForm && (target.matches('input[type="text"], input[type="search"], input[data-search-input]') || target.matches('select'))) {
        e.preventDefault();
        filterForm.submit();
      }
    }
