...
<body>
    ...
    <main class="md:ml-[280px] min-h-screen p-4 md:p-8 transition-all">
        ...
        <section class="mb-6">
            <div class="flex flex-wrap items-center justify-between gap-2 mb-4">
                ...
                <div class="flex gap-2">
                    <button type="button" data-open-modal="vendorHistoryModal"
        ...
        <section class="mb-6">
            ...
            <form data-filter method="get" class="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm mb-4">
                <input type="hidden" name="route" value="admin/gestion-vendeur">
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            <?php if (empty($vendors)): ?>
                                <tr>
                                    <td colspan="10" class="px-4 py-6 text-center text-sm text-gray-400">Aucun vendeur enregistré.</td>
                                </tr>
                                    ...
                                    <tr class="hover:bg-gray-50/50 transition">
                                        ...
                                        <td class="px-4 py-3">
                                            <div class="flex items-center justify-end gap-1">
                                                ...
                                                <button type="button" data-open-modal="vendorHistoryModal" data-id="<?= (int) $v['id'] ?>"
                                                    class="w-8 h-8 rounded-lg bg-indigo-50 text-indigo-600 hover:bg-indigo-100 flex items-center justify-center" title="Historique">
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            <?php if (empty($vendorsWithBalance)): ?>
                                <tr>
                                    <td colspan="4" class="px-4 py-6 text-center text-sm text-gray-400">Aucun versement en attente.</td>
                                </tr>
    ...
    <!-- MODAL : HISTORIQUE VENDEURS -->
    <div id="vendorHistoryModal" class="fixed inset-0 z-[60] hidden items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-3xl max-h-[92vh] overflow-hidden flex flex-col">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                <div>
                    <h3 class="text-lg font-bold text-[#0F172A] flex items-center gap-2">
                        <i class="fas fa-history text-[#0EA486]"></i> Historique vendeurs
                    </h3>
                    <p class="text-xs text-gray-400">Décisions et actions administrateur</p>
                </div>
            ...
            <div class="p-6 overflow-y-auto">
                <div data-vendor-history-list class="space-y-2">
    ...
    <script>
        ...
        (function() {
            document.body.addEventListener('click', function(e) {
                ...
                let payload;
                try {
                    payload = JSON.parse(trigger.getAttribute('data-vendor'));
                } catch (_) {
                    return;
                }
        ...
        // 6. Historique vendeurs
        (function() {
            const modal = document.getElementById('vendorHistoryModal');
            const list = modal ? modal.querySelector('[data-vendor-history-list]') : null;
            ...
            document.body.addEventListener('click', function(e) {
                const trigger = e.target.closest('[data-open-modal="vendorHistoryModal"]');
                if (!trigger) return;
                const id = trigger.getAttribute('data-id') || '';
                const url = window.NDIGIT_BASE_URL + '/index.php?route=admin/getVendorDecisionHistory' + (id ? '&id=' + encodeURIComponent(id) : '');
                        ...
                        list.innerHTML = data.history.map(item => `
                            <div class="p-3 rounded-xl border border-gray-100 bg-gray-50">
                                <div class="flex flex-wrap items-start justify-between gap-2">
                                    <div>
                                        <p class="text-sm font-semibold text-[#0F172A]">${escapeHtml(item.action_label || item.action || 'Action vendeur')}</p>
                                        <p class="text-xs text-gray-500">${escapeHtml(item.details || '')}</p>
                                    </div>
