...
<body>
    <?php $currentPage = 'financieres-commission';
        ...
        <header class="flex items-center justify-between mb-6 bg-white/80 backdrop-blur-sm sticky top-0 z-30 py-4 px-4 md:px-6 rounded-2xl shadow-sm border border-gray-100/50">
            ...
            <div class="flex items-center gap-3">
                <form data-filter method="get" class="flex gap-2">
                    ...
                    <input type="hidden" name="route" value="admin/financieres-commission">
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            <?php if (empty($commissionsByCategory)): ?>
                                <tr>
                                    <td colspan="6" class="px-4 py-6 text-center text-sm text-gray-400">Aucune donnée</td>
                                </tr>
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            <?php if (empty($commissionsByVendor)): ?>
                                <tr>
                                    <td colspan="7" class="px-4 py-6 text-center text-sm text-gray-400">Aucun vendeur avec des ventes</td>
                                </tr>
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            <?php if (empty($refunds)): ?>
                                <tr>
                                    <td colspan="9" class="px-4 py-6 text-center text-sm text-gray-400">Aucun remboursement</td>
                                </tr>
                                        ...
                                        <td class="px-4 py-3">
                                            <div class="flex items-center justify-end gap-2">
                                                <button data-open-modal="refundDetailModal" data-id="<?= (int)$r['id'] ?>"
                                                    class="w-8 h-8 rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-100 flex items-center justify-center" title="Voir les détails">
                                                    <i class="fas fa-eye text-xs"></i>
                                                </button>
                                                <button data-open-modal="refundEditModal" data-id="<?= (int)$r['id'] ?>"
                                                    class="w-8 h-8 rounded-lg bg-gray-50 text-gray-600 hover:bg-gray-100 flex items-center justify-center" title="Modifier">
                                                    <i class="fas fa-pen text-xs"></i>
                                                </button>
                                                <button type="button" data-action="admin/deleteRefund" data-id="<?= (int)$r['id'] ?>"
                                                    data-confirm="Supprimer ce remboursement ?" data-success="Remboursement supprimé"
        ...
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
            <form data-ajax data-action="admin/createRefund" data-success="Remboursement initié">
                ...
                <div class="p-6 space-y-4">
                    <div>
                        <label class="text-xs font-semibold text-gray-600 mb-1 block">ID ou code commande <span class="text-red-500">*</span></label>
                        <input type="text" name="commande_id" required class="w-full px-3 py-2.5 bg-gray-50 border border-gray-100 rounded-xl text-sm" placeholder="Ex: 12 ou CMD_68d...">
                    </div>
                    <div>
                    ...
                    <div>
                        <label class="text-xs font-semibold text-gray-600 mb-1 block">Statut <span class="text-red-500">*</span></label>
                        <select name="statut" required class="w-full px-3 py-2.5 bg-gray-50 border border-gray-100 rounded-xl text-sm">
    ...
    <!-- ============ MODAL : MODIFIER REMBOURSEMENT ============ -->
    <div id="refundEditModal" class="fixed inset-0 z-[60] hidden items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-md max-h-[92vh] overflow-hidden flex flex-col">
            <form data-ajax data-action="admin/updateRefund" data-success="Remboursement mis à jour" class="flex flex-col max-h-[92vh]">
                <input type="hidden" name="id">
                <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                    <div>
                        <h3 class="text-lg font-bold text-[#0F172A] flex items-center gap-2"><i class="fas fa-pen text-[#0EA486]"></i> Modifier le remboursement</h3>
                        <p class="text-xs text-gray-400">Mettre à jour le montant, la commission, le motif et le statut</p>
                    </div>
                    <button type="button" data-close-modal class="w-9 h-9 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center text-gray-600"><i class="fas fa-times"></i></button>
                </div>
                <div class="p-6 overflow-y-auto space-y-4">
                    ...
                    <div>
                        <label class="text-xs font-semibold text-gray-600 mb-1 block">Commission récupérée (FCFA)</label>
                        <input type="number" name="commission_recuperee" min="0" step="0.01" class="w-full px-3 py-2.5 bg-gray-50 border border-gray-100 rounded-xl text-sm">
                    </div>
                    <div>
                        <label class="text-xs font-semibold text-gray-600 mb-1 block">Statut <span class="text-red-500">*</span></label>
                        <select name="statut" required class="w-full px-3 py-2.5 bg-gray-50 border border-gray-100 rounded-xl text-sm">
                    ...
                    <div>
                        <label class="text-xs font-semibold text-gray-600 mb-1 block">Motif <span class="text-red-500">*</span></label>
                        <textarea name="motif" rows="4" required class="w-full px-3 py-2.5 bg-gray-50 border border-gray-100 rounded-xl text-sm resize-none"></textarea>
                    </div>
                </div>
                <div class="px-6 py-4 border-t border-gray-100 bg-gray-50/50 flex items-center justify-end gap-2">
                    <button type="button" data-close-modal class="px-4 py-2.5 rounded-xl bg-gray-100 text-gray-700 text-sm">Annuler</button>
                    <button type="submit" class="px-5 py-2.5 rounded-xl bg-[#0EA486] hover:bg-[#0c8f75] text-white text-sm font-semibold"><i class="fas fa-save"></i> Enregistrer</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        const financialPeriod = <?= json_encode($periode ?? 'all') ?>;
        const financialPeriodParam = '&periode=' + encodeURIComponent(financialPeriod);
        ...
        (function() {
            document.body.addEventListener('click', function(e) {
                ...
                if (modalId === 'categoryReportModal' && id) {
                    ...
                    fetch(window.NDIGIT_BASE_URL + '/index.php?route=admin/getCategoryReport&categorie_id=' + id + financialPeriodParam)
                        .then(r => r.json())
                        .then(data => {
                            if (!data.success) {
                                box.innerHTML = '<div class="text-red-500">' + escapeHtml(data.message || 'Erreur') + '</div>';
                                return;
                            }
                            let html = `
                            <div class="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl p-5 border border-blue-100">
                                <h4 class="text-lg font-bold">${escapeHtml(data.category.nom_categorie)}</h4>
                                <div class="grid grid-cols-3 gap-4 mt-3">
                                    <div><p class="text-xs text-gray-500">CA total</p><p class="text-xl font-bold">${escapeHtml(data.ca_formatted)}</p></div>
                                    <div><p class="text-xs text-gray-500">Commission</p><p class="text-xl font-bold text-[#0EA486]">${escapeHtml(data.commission_formatted)}</p></div>
                                    <div><p class="text-xs text-gray-500">Ventes</p><p class="text-xl font-bold">${escapeHtml(data.category.nb_ventes)}</p></div>
                                </div>
                            </div>
                            <div class="bg-white rounded-2xl p-4 border border-gray-100">
                            ...
                            if (!data.products || data.products.length === 0) {
                                html += '<tr><td colspan="4" class="py-4 text-center text-sm text-gray-400">Aucun produit vendu sur cette période.</td></tr>';
                            } else {
                                data.products.forEach(p => {
                                    html += `<tr class="border-t border-gray-100">
                                    <td class="py-2 text-sm font-medium">${escapeHtml(p.nom_article)}</td>
                                    <td class="text-xs text-gray-500">${escapeHtml((p.vendeur_prenom || '') + ' ' + (p.vendeur_nom || ''))}</td>
                                    <td class="text-xs font-semibold">${escapeHtml(p.nb_ventes)}</td>
                                    <td class="text-xs font-semibold">${money(p.ca)}</td>
                                </tr>`;
                                });
                            }
                        ...
                        .catch(() => {
                            box.innerHTML = '<div class="text-red-500">Erreur de chargement</div>';
                        });
                }
                ...
                if (modalId === 'vendorReportModal' && id) {
                    ...
                    box.innerHTML = '<div class="p-6 text-center text-gray-400"><i class="fas fa-spinner fa-spin mr-2"></i>Chargement...</div>';
                    fetch(window.NDIGIT_BASE_URL + '/index.php?route=admin/getVendorReport&vendeur_id=' + id + financialPeriodParam)
                        .then(r => r.json())
                        .then(data => {
                            if (!data.success) {
                                box.innerHTML = '<div class="text-red-500">' + escapeHtml(data.message || 'Erreur') + '</div>';
                            ...
                            <div class="bg-gradient-to-br from-emerald-50 to-teal-50 rounded-2xl p-5 border border-emerald-100">
                                <h4 class="text-lg font-bold">${escapeHtml((v.prenom || '') + ' ' + (v.nom || ''))}</h4>
                                <p class="text-xs text-gray-500">${escapeHtml(v.email || '')}</p>
                                <div class="grid grid-cols-4 gap-4 mt-3">
                                    <div><p class="text-xs text-gray-500">CA total</p><p class="text-xl font-bold">${escapeHtml(data.ca_formatted)}</p></div>
                                    <div><p class="text-xs text-gray-500">Net vendeur</p><p class="text-xl font-bold text-[#0EA486]">${escapeHtml(data.net_formatted)}</p></div>
                                    <div><p class="text-xs text-gray-500">Solde dû</p><p class="text-xl font-bold text-orange-600">${escapeHtml(data.solde_formatted)}</p></div>
                                    <div><p class="text-xs text-gray-500">Ventes</p><p class="text-xl font-bold">${escapeHtml(v.nb_ventes)}</p></div>
                                </div>
                            </div>
                            <div class="bg-white rounded-2xl p-4 border border-gray-100">
                                ...
                                <table class="w-full text-sm">
                                    ...
                                    <tbody>`;
                            if (!data.products || data.products.length === 0) {
                                html += '<tr><td colspan="5" class="py-4 text-center text-sm text-gray-400">Aucun produit vendu sur cette période.</td></tr>';
                            } else {
                                data.products.forEach(p => {
                                    html += `<tr class="border-t border-gray-100">
                                    <td class="py-2 text-sm font-medium">${escapeHtml(p.nom_article)}</td>
                                    <td class="text-xs">${money(p.prix)}</td>
                                    <td class="text-xs">${escapeHtml(p.statut)}</td>
                                    <td class="text-xs font-semibold">${escapeHtml(p.nb_ventes)}</td>
                                    <td class="text-xs font-semibold">${money(p.ca)}</td>
                                </tr>`;
                                });
                ...
                // Charger détail / édition remboursement
                if ((modalId === 'refundDetailModal' || modalId === 'refundEditModal') && id) {
                    fetch(window.NDIGIT_BASE_URL + '/index.php?route=admin/getRefundDetails&id=' + id)
                        .then(r => r.json())
                        .then(data => {
                            if (!data.success) return;
                            if (modalId === 'refundDetailModal') {
                            ...
                            modal.querySelectorAll('[name]').forEach(field => {
                                if (data.refund[field.name] !== undefined) {
                                    field.value = data.refund[field.name] ?? '';
                                }
                            });
                        });
                }
        ...
        (function() {
            ...
            btn.addEventListener('click', function() {
                ...
                fetch(window.NDIGIT_BASE_URL + '/index.php?route=admin/getMonthlyReport&mois=' + mois)
                    ...
                    .then(data => {
                        if (!data.success) {
                            box.innerHTML = '<div class="text-red-500">Erreur</div>';
                            return;
                        }
                        const r = data.report;
                        let html = `
