...
<body class="bg-gray-50">
    ...
    <main class="md:ml-[280px] min-h-screen p-3 sm:p-4 md:p-8 transition-all">

        <!-- HEADER -->
        <header class="flex items-center justify-between gap-3 mb-6 bg-white/80 backdrop-blur-sm sticky top-0 z-30 py-3 px-3 sm:py-4 sm:px-4 md:px-6 rounded-2xl shadow-sm border border-gray-100/50">
            <div class="flex items-center gap-3 min-w-0">
                ...
                <div class="min-w-0">
                    <h2 class="text-lg sm:text-xl md:text-2xl font-bold text-[#0F172A] truncate">Gestion utilisateur</h2>
        ...
        <section class="mb-6">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
                ...
                <div class="grid grid-cols-2 sm:flex gap-2 w-full sm:w-auto">
                    <a href="<?= htmlspecialchars($csvUrl) ?>" class="px-3 py-2 rounded-lg bg-gray-100 text-gray-600 hover:bg-gray-200 text-xs font-medium flex items-center justify-center gap-2">
                        <i class="fas fa-file-csv"></i> Export CSV
                    </a>
                    <a href="<?= htmlspecialchars($excelUrl) ?>" class="px-3 py-2 rounded-lg bg-gray-100 text-gray-600 hover:bg-gray-200 text-xs font-medium flex items-center justify-center gap-2">
                        <i class="fas fa-file-excel"></i> Export Excel
                    </a>
                    <button type="button" id="openFormBtn" data-open-modal="formModal" class="col-span-2 sm:col-span-1 px-4 py-2 rounded-lg bg-[#0EA486] hover:bg-[#0c8f75] text-white text-xs font-semibold flex items-center justify-center gap-2 shadow-sm transition">
            ...
            <form method="GET" data-filter class="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm mb-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-[minmax(260px,1fr)_180px_180px_auto] gap-3">
                    ...
                    <select name="type" class="w-full px-3 py-2.5 bg-gray-50 border border-gray-100 rounded-xl text-sm text-gray-600 focus:outline-none focus:border-[#0EA486]">
                    ...
                    <select name="statut" class="w-full px-3 py-2.5 bg-gray-50 border border-gray-100 rounded-xl text-sm text-gray-600 focus:outline-none focus:border-[#0EA486]">
                    ...
                    <a href="?route=admin/gestion-utilisateurs" class="px-3 py-2.5 rounded-xl bg-gray-100 hover:bg-gray-200 text-gray-600 text-sm flex items-center justify-center gap-2">
                ...
                <div data-bulk-bar class="hidden flex-wrap items-center gap-2 mt-3 pt-3 border-t border-gray-100">
                    ...
                    <button type="button" data-open-modal="emailModal" data-email-from-selection class="px-3 py-1.5 rounded-lg bg-indigo-50 text-indigo-600 hover:bg-indigo-100 text-xs font-medium flex items-center gap-1.5"><i class="fas fa-envelope"></i> Envoyer un email</button>
                            ...
                            <?php if (!empty($users)) : ?>
                                <?php foreach ($users as $user) : ?>
                                    ...
                                    <tr class="border-b border-gray-100 hover:bg-gray-50/60">
                                        ...
                                        <td class="px-4 py-4 text-gray-700 max-w-[220px] truncate"><?= htmlspecialchars($user['email'] ?? '') ?></td>
                                        ...
                                        <td class="px-4 py-4 text-right">
                                            <div class="inline-flex items-center gap-1">
                                                ...
                                                <?php endif; ?>
                                                <button type="button" data-open-modal="emailModal" data-email-user-id="<?= $uid ?>" class="px-2 py-1.5 rounded-lg bg-indigo-50 hover:bg-indigo-100 text-indigo-600 text-xs" title="Envoyer un email"><i class="fas fa-envelope"></i></button>
        ...
        <section class="mb-8">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                ...
                <button type="button" data-open-modal="emailModal" data-email-from-selection class="text-left bg-white rounded-2xl p-4 border border-gray-100 shadow-sm hover:border-[#0EA486] transition group lg:col-span-2">
    ...
    <!-- MODAL FORMULAIRE -->
    <div id="formModal" class="fixed inset-0 z-50 hidden items-center justify-center p-3 sm:p-4 bg-black/40 backdrop-blur-sm">
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
            <div class="flex items-center justify-between gap-3 px-4 sm:px-6 py-4 border-b border-gray-100">
                <div class="min-w-0">
                    <h3 class="text-base sm:text-lg font-bold text-[#0F172A] truncate">Nouvel utilisateur</h3>
            ...
            <form data-ajax data-action="admin/createUser" data-success="Utilisateur enregistré" class="p-4 sm:p-6 overflow-y-auto space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select name="type" required class="w-full px-3 py-2.5 bg-gray-50 border border-gray-100 rounded-xl text-sm focus:outline-none focus:border-[#0EA486]">
                            ...
                            <option value="pro">Vendeur </option>
                ...
                <div class="flex flex-col-reverse sm:flex-row sm:items-center sm:justify-end gap-2 pt-4 border-t border-gray-100">
                    <button type="button" id="cancelFormBtn" data-close-modal="formModal" class="w-full sm:w-auto px-4 py-2.5 rounded-xl bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm font-medium">
                        Annuler
                    </button>
                    <button type="submit" class="w-full sm:w-auto px-5 py-2.5 rounded-xl bg-[#0EA486] hover:bg-[#0c8f75] text-white text-sm font-semibold flex items-center justify-center gap-2 shadow-sm">
    ...
    <div id="emailModal" class="fixed inset-0 z-50 hidden items-center justify-center p-3 sm:p-4 bg-black/40 backdrop-blur-sm">
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-6xl max-h-[92vh] overflow-hidden flex flex-col">
            <div class="flex items-center justify-between gap-3 px-4 sm:px-6 py-4 border-b border-gray-100">
                <div class="min-w-0">
                    <h3 class="text-base sm:text-lg font-bold text-[#0F172A] truncate">Envoyer un email personnalisé</h3>
            ...
            <form data-ajax data-action="admin/sendCustomEmail" data-success="Email envoyé" class="overflow-hidden flex flex-col">
                <div class="grid grid-cols-1 lg:grid-cols-[380px_1fr] min-h-0 overflow-y-auto lg:overflow-hidden">
                    <section class="border-b lg:border-b-0 lg:border-r border-gray-100 p-4 sm:p-5 min-h-0">
                        ...
                        <div class="max-h-[32vh] lg:max-h-[54vh] overflow-y-auto border border-gray-100 rounded-xl divide-y divide-gray-100">
                            <?php foreach (($users ?? []) as $emailUser): ?>
                                ...
                                <label data-email-user="<?= htmlspecialchars(mb_strtolower($emailName . ' ' . $emailAddress . ' ' . $emailUid)) ?>" class="flex items-center gap-3 px-3 py-3 hover:bg-gray-50 cursor-pointer">
                                    ...
                                    <span class="min-w-0 flex-1">
                                        <span class="block text-sm font-semibold text-[#0F172A] truncate"><?= htmlspecialchars($emailName ?: 'Utilisateur #' . $emailUid) ?></span>
                                        <span class="block text-sm font-medium text-[#0EA486] truncate"><?= htmlspecialchars($emailAddress) ?></span>
                                    </span>
                                </label>
                            <?php endforeach; ?>
                        </div>
                    ...
                    <section class="p-4 sm:p-5 overflow-y-visible lg:overflow-y-auto min-h-0 space-y-4">
                        <div>
                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-4 sm:px-6 py-4 border-t border-gray-100 bg-gray-50/50">
                    ...
                    <div class="flex flex-col-reverse sm:flex-row sm:items-center gap-2 w-full sm:w-auto">
                        <button type="button" data-close-modal="emailModal" class="w-full sm:w-auto px-4 py-2.5 rounded-xl bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm font-medium">Annuler</button>
                        <button type="submit" class="w-full sm:w-auto px-5 py-2.5 rounded-xl bg-[#0EA486] hover:bg-[#0c8f75] text-white text-sm font-semibold flex items-center justify-center gap-2 shadow-sm">
    ...
    <script>
        ...
        (function() {
            ...
            const tableSelection = () => Array.from(document.querySelectorAll('[data-bulk-checkbox]:checked')).map(input => input.value);
            ...
            document.querySelectorAll('[data-open-modal="emailModal"]').forEach(btn => {
                btn.addEventListener('click', function() {
                    const singleId = this.getAttribute('data-email-user-id');
                    const ids = singleId ? [singleId] : tableSelection();
                    selectRecipients(ids);
                });
            });
