...
trait OrdersTrait
{
    ...
    public function orders()
    {
        ...
        if (!empty($_GET['search'])) {
            $where[] = "(c.a LIKE :search OR c.email LIKE :search OR p.nom_article LIKE :search OR u.nom LIKE :search OR u.prenom LIKE :search)";
            $params[':search'] = '%' . $_GET['search'] . '%';
        }

        // Filtre par vendeur
        if (!empty($_GET['vendeur']) && $_GET['vendeur'] !== '') {
            $where[] = "p.id_vendeur = :vendeur";
            $params[':vendeur'] = $_GET['vendeur'];
        }
        ...
        foreach ($orders as &$order) {
            $order['prix_formatted'] = $this->formatCurrency((float)($order['prix'] ?? 0));
            $order['commission_plateforme'] = round((float)($order['prix'] ?? 0) * 0.10, 2);
            $order['commission_vendeur'] = round((float)($order['prix'] ?? 0) * 0.90, 2);
        ...
        $totalRevenue = array_sum(array_column($orders, 'prix'));
    ...
    public function getOrderDetails(): void
    {
        ...
        $order['prix_formatted'] = $this->formatCurrency((float)$order['prix']);
        $order['commission_plateforme'] = round((float)$order['prix'] * 0.10, 2);
        $order['commission_vendeur'] = round((float)$order['prix'] * 0.90, 2);
        $order['commission_plateforme_formatted'] = $this->formatCurrency($order['commission_plateforme']);
        $order['commission_vendeur_formatted'] = $this->formatCurrency($order['commission_vendeur']);
        $order['date_formatted'] = $this->formatDate($order['date_commande']);
        $order['vendeur_full_name'] = trim(($order['vendeur_prenom'] ?? '') . ' ' . ($order['vendeur_nom'] ?? ''));
    ...
    public function createOrder(): void
    {
        ...
        if (!$idArticle || $prix <= 0) {
            $this->jsonResponse(['success' => false, 'message' => 'Produit et prix requis'], 400);
            return;
        }
        ...
        $stmt = $db->prepare("
            INSERT INTO commande (id_client, email, id_article, image, prix, fichier, date_commande)
            VALUES (?, ?, ?, ?, ?, ?, NOW())
        ");
        $stmt->execute([
            $idClient ?: null,
